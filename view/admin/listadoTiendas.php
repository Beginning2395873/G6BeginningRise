<?php
    ...
    $query = $connect->conexion->prepare(
        "SELECT count(*) as totalRegistros from tiendas"
    );
    $query->execute();
    $arreglo = $query->fetch();
    ...
    if (!empty($_POST['search'])) 
    {
        ...
        $cuentaBusqueda = $connect->conexion->prepare(
            "SELECT count(tiendas.nit_tienda) as totalBusqueda, 
            ...
            tiendas.estado 
            FROM tiendas 
            where nit_tienda like '%$busqueda%' 
            or nombre_tienda like '%$busqueda%'"
        );
                            ...
                            ?>
                                ...
                                ?>
                                    <tr>
                                        ...
                                        <td>
                                            <img class="rounded-circle" src="<?php echo $arreglo2[5] ?>" alt="" height="80px" width="80px">
                                        </td>
                                        <td>
                                            <?php echo $status ?>
                                        </td>
                                        <?php if($status=='Activo'){?>
                                            <td>
                                                <?php echo "
                                                    <form id='desactivarTienda' action='".urlsite."?page=eliminarTienda' name='desactivarTienda' method='POST'>
                                                        <input type='hidden' id='nit' name='nit' value='$arreglo2[0]' />
                                                        <button type='submit' name='eliminar' class='btn btn-warning' onClick='alertaTiendaDesactivar()' >
                                        ...
                                        <?php } else if($status=='Inactivo') {?>
                                            <td>
                                                <?php echo "
                                                    <form id='desactivarTienda' action='".urlsite."?page=eliminarTienda' name='desactivarTienda' method='POST'>
                                                        <input type='hidden' id='nit' name='nit' value='$arreglo2[0]' />
                                                        <button type='submit' name='eliminar' class='btn btn-success' onClick='alertaTiendaActivar()' >

<?php
    ...
    $query = $connect->conexion->prepare(
        "SELECT count(*) as totalRegistros from clientes"
    );
    $query->execute();
    $arreglo = $query->fetch();
    ...
    if (!empty($_POST['search'])) {
        ...
        $sqlcuentaBusqueda = $connect->conexion->prepare(
            "SELECT count(persona.tipo_documento_persona) as totalBusqueda, 
            persona.num_doc_persona, persona.nombre_persona, 
            ...
            persona.estado 
            FROM clientes 
            INNER JOIN persona ON persona.email_persona = clientes.email_cliente 
            where tipo_documento_persona like '%$busqueda%' 
            or num_doc_persona like '%$busqueda%' 
            or nombre_persona like '%$busqueda%'"
        );
                            ...
                            ?>
                                ...
                                ?>
                                    <tr>
                                        ...
                                        <td>
                                            <img src="<?php echo $arreglo2[6] ?>" alt="" width="80px">
                                        </td>
                                        <td>
                                            <?php echo $status ?>
                                        </td>
